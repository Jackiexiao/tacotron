...
from datasets import blizzard, ljspeech, thchs30, king003
...
def preprocess_king003(args):
  in_dir = os.path.join(args.base_dir, 'king003')
  out_dir = os.path.join(args.base_dir, args.output)
  os.makedirs(out_dir, exist_ok=True)
  metadata = king003.build_from_path(in_dir, out_dir, args.num_workers, tqdm=tqdm)
  write_metadata(metadata, out_dir)
...
def main():
  ...
  parser.add_argument('--dataset', required=True, choices=['blizzard', 'ljspeech', 'thchs30', 'king003'])
  ...
  elif args.dataset == 'king003':
    preprocess_king003(args)
